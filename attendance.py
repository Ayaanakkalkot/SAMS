...
class Attendance:
    ...
    def get_cursor_right(self, event=""):
        ...
        # Check if data is not empty and has enough elements before accessing them
        if data and len(data) >= 6:  # Ensure there are at least 6 elements in data
            self.var_id.set(data[0])
            self.var_roll.set(data[1])
            self.var_name.set(data[2])
            self.var_time.set(data[3])
            ...
            # Handle the case where data is empty or has fewer elements than expected
            print("Data is empty or incomplete:", data)
            # You can also reset the fields if necessary
            self.var_id.set("")
            self.var_roll.set("")
            self.var_name.set("")
            self.var_time.set("")
